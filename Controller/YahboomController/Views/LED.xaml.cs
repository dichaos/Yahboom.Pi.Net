...
namespace YahboomController.Views
{
    public class LED : UserControl
    {
        ...
        public LED()
        {
            ...
            _RedSlider.PointerCaptureLost += async (sender, args) => { await ChangeRed(); };

            _GreenSlider.PointerCaptureLost += async (sender, args) => { await ChangeGreen(); };

            _BlueSlider.PointerCaptureLost += async (sender, args) => { await ChangeBlue(); };
        }
        ...
        private async Task ChangeRed()
        {
            ...
            _ColorTextBlock.Background = new SolidColorBrush(new Color(255, (byte) _RedSlider.Value,
                (byte) _GreenSlider.Value, (byte) _BlueSlider.Value));
        ...
        {
            var client = ((ClientViewModel) DataContext)?.Client;
            if (client != null)
                await client.SetLED((int) _RedSlider.Value, (int) _GreenSlider.Value, (int) _BlueSlider.Value);
        }
    }
}